...
public abstract class XMPPMessageProcessor {
		...
		try {
			...
			while ((event = asyncReader.next()) != AsyncXMLStreamReader.EVENT_INCOMPLETE) {
				...
				case AsyncXMLStreamReader.START_DOCUMENT:
					...
					markLastEvent(getCurrentLocation());break;
				case AsyncXMLStreamReader.START_ELEMENT:
					handleStartElement(getCurrentLocation());
					markLastEvent(getCurrentLocation());break;
				case AsyncXMLStreamReader.END_ELEMENT:
					handleEndElement(getCurrentLocation());break;
	...
	public String getEventString(int vLocation) {
		return this.buffer.substring(normalizeLocation(this.lastEvent), normalizeLocation(vLocation));
	}
	...
	protected void resetReader() {
		...
		System.out.println("Nuevo Buffer: " + this.buffer);
